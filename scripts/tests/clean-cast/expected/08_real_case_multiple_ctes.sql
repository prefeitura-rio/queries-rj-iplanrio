...
WITH
    ...
    secretaria AS (
        SELECT
            {{ clean_and_cast('cd_ua', 'int64', trim=false) }} AS id_secretaria,
            {{ clean_and_cast('sigla_ua', 'string', trim=false) }} AS secretaria_sigla,
            {{ clean_and_cast('nome_ua', 'string', trim=false) }} AS secretaria_nome
