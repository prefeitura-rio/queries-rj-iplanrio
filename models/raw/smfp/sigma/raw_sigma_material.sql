...
SELECT
  ...
  SAFE_CAST(DT_DESATIVACAO AS NUMERIC) AS data_desativacao,
  ...
  SAFE_CAST(CD_MATERIAL_SUBSTITUTO AS NUMERIC) AS codigo_material_substituto,
  SAFE_CAST(NM_FANTASIA AS STRING) AS nome_fantasia,
  SAFE_CAST(CD_COMPRASNET AS NUMERIC) AS codigo_compras_net,
...
FROM {{ source('brutos_compras_materiais_servicos_sigma_staging', 'material') }}