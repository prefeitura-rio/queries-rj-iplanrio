...
SELECT
  ...
  SAFE_CAST(INSCRICAO_ESTADUAL AS NUMERIC) AS inscricao_estadual,
  ...
  SAFE_CAST(FAX AS NUMERIC) AS fax_fornecedor,
  SAFE_CAST(DDD AS NUMERIC) AS ddd,
  SAFE_CAST(DDI AS NUMERIC) AS ddi,
  SAFE_CAST(RAMAL AS NUMERIC) AS ramal,
  SAFE_CAST(TELEFONE AS NUMERIC) AS telefone,
  ...
  SAFE_CAST(CD_NATUREZA_JURIDICA AS NUMERIC) AS codigo_natureza_juridica,
  SAFE_CAST(DS_NATUREZA_JURIDICA AS STRING) AS descricao_natureza_juridica,
  SAFE_CAST(CD_RAMO_ATIVIDADE AS NUMERIC) AS codigo_ramo_atividade,
  SAFE_CAST(DS_RAMO_ATIVIDADE AS STRING) AS descricao_ramo_atividade,
  SAFE_CAST(CD_PORTE_EMPRESA AS NUMERIC) AS codigo_porte_fornecedor,
