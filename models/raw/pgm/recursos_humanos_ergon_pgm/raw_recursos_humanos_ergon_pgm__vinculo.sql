...
inner join (
    select distinct id_empresa from {{ ref('raw_recursos_humanos_ergon_pgm__setor')}} 
...
inner join (
    select distinct id_funcionario, id_vinculo from {{ ref('raw_recursos_humanos_ergon_pgm__provimento')}} 
