{{
    config(
        alias='avaliacao_bimestral_2021_a_2026',
        schema='educacao_basica_avaliacao_backup',
        materialized='table'
    )
}}


WITH d2021_a_2026 AS
(
  WITH d2021 as (
SELECT
...
SELECT
  ...
  SAFE_CAST(dc_habilidade_acerto AS STRING) as habilidades_avaliadas_acertos,
  SAFE_CAST(dc_habilidade_total AS STRING) as habilidades_avaliadas_total
FROM `rj-sme.educacao_basica_avaliacao_staging.bimestral_2026`
)
