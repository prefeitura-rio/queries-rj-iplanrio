...
SELECT
    ...
    SAFE_CAST(TRIM(num_folha) AS INT64) AS num_folha,
    SAFE_CAST(REGEXP_REPLACE(TRIM(setor), r'.0$', '') AS STRING) AS setor,
    SAFE_CAST(TRIM(secretaria) AS STRING) AS secretaria,
    SAFE_CAST(TRIM(tipo_func) AS STRING) AS tipo_func,
    SAFE_CAST(TRIM(detalha) AS STRING) AS detalha,
    ...
    SAFE_CAST(TRIM(tipo_classif) AS STRING) AS tipo_classif,
...
FROM {{ source('brutos_ergon_saude_staging', 'IPL_PT_FICHAS') }} AS t
-- WHERE
