...
{% if is_incremental() and execute %}
    ...
    {% else %} {% set start_date = "2023-01-01" %}  -- valor default para compilação
    {% endif %}
{% else %}
    {% set start_date = (
        modules.datetime.datetime.utcnow()
        - modules.datetime.timedelta(days=180)
    ).strftime("%Y-%m-%d") %}
{% endif %}
