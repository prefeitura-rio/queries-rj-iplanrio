...
with
    ...
    parsed_logs as (
        ...
        select
            ...
            coalesce(json_extract_scalar(message, '$.is_err'), 'false')
            = 'true' as is_error,
            ...
            cast(
                json_extract_scalar(
                    message, '$.usage_metadata.candidates_token_count'
                ) as int64
            ) as candidates_tokens,
            cast(
                json_extract_scalar(
                    message, '$.usage_metadata.total_token_count'
                ) as int64
            ) as total_tokens,
            coalesce(
    ...
    tool_executions as (
        ...
        select
            ...
            -- CORREÇÃO: Define explicitamente o tipo do NULL para corresponder à
            -- outra tabela
            ...
            coalesce(c.candidates_tokens, 0)
            + coalesce(r.candidates_tokens, 0) as candidates_tokens,
...
-- Passo Final: Combina as linhas de execução de ferramenta com as outras mensagens
select *
