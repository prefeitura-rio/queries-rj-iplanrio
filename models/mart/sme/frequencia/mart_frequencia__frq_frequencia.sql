        ...
        with Turmas as (
            ...
            FROM {{ ref('_raw_gestao_escolar__tur_turma') }} tur
            INNER JOIN {{ ref('_raw_gestao_escolar__turma_disciplina_rel') }} rtd
                ON rtd.id_turma = tur.tur_id AND tur_situacao = 1
            INNER JOIN {{ ref('_raw_gestao_escolar__turma_disciplina') }} tud
            ...
            INNER JOIN {{ ref('_raw_gestao_escolar__turno') }} trn
                ON tur.trn_id = trn.id_turno AND situacao = 1
            INNER JOIN {{ ref('_raw_gestao_escolar__esc_escola') }} esc
                ON tur.esc_id = esc.id_esc AND esc_situacao = 1
            INNER JOIN {{ ref('_raw_gestao_escolar__turma_curriculo') }} tcr
        ...
        FrequenciaDia as (
            ...
            INNER JOIN {{ ref('_raw_gestao_escolar__turma_aula_aluno') }} taa
