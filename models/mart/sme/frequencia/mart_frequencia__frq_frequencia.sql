        ...
        with Turmas as (
            ...
            FROM {{ ref('gestao_escolar__tur_turma') }} tur
            INNER JOIN {{ ref('gestao_escolar__turma_disciplina_rel') }} rtd
                ON rtd.id_turma = tur.tur_id AND tur_situacao = 1
            INNER JOIN {{ ref('gestao_escolar__turma_disciplina') }} tud
                ON tud.id_disciplina_turma = rtd.id_disciplina AND id_situacao = '1'
            INNER JOIN {{ ref('gestao_escolar__formato_avaliacao') }} fav
                ON fav.id_formato_avaliacao = tur.fav_id AND situacao_registro <> 3
            INNER JOIN {{ ref('gestao_escolar__turno') }} trn
                ON tur.trn_id = trn.id_turno AND situacao = 1
            INNER JOIN {{ ref('gestao_escolar__esc_escola') }} esc
                ON tur.esc_id = esc.id_esc AND esc_situacao = 1
            INNER JOIN {{ ref('gestao_escolar__turma_curriculo') }} tcr
        ...
        FrequenciaDia as (
            ...
            FROM Turmas tur
            INNER JOIN {{ ref('gestao_escolar__turma_aula') }} tau
                ON tau.id_disciplina = tur.id_disciplina_turma
                AND EXTRACT(YEAR FROM tau.data_aula) = 2026
            INNER JOIN {{ ref('gestao_escolar__turma_aula_aluno') }} taa
            ...
            INNER JOIN {{ ref('gestao_escolar__mtr_matricula_turma') }} mtu
