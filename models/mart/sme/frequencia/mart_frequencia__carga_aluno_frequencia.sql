...
with frqtrn as (
    ...
    from {{ ref('gestao_escolar__turno_horario') }}
    ...
    from (
        ...
        inner join {{ ref('gestao_escolar__aluno_curriculo') }} alc
        ...
        inner join {{ ref('gestao_escolar__diascoc')}} dia
...
select
    ...
    faltas_coc5,
    nm_aulas_coc5
from {{ ref('gestao_escolar__vw_bi_aluno') }} alu
inner join {{ ref('gestao_escolar__tur_turma') }} tur on alu.tur_id = tur.tur_id and tur.tur_situacao = 1
inner join {{ ref('gestao_escolar__esc_escola') }} esc on esc.id_esc = tur.esc_id and esc.esc_situacao = 1
