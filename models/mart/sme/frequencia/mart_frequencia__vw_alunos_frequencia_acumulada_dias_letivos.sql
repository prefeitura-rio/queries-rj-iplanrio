...
WITH frequencia_acumulada AS (
    ...
    FROM {{ ref('gestao_escolar__numeroDeAulasCte') }} NUM
    LEFT JOIN (
        SELECT
            MTU.alu_id
        ...
        FROM {{ ref('gestao_escolar__turma_aula_aluno') }} TAA
    ...
    INNER JOIN {{ ref('gestao_escolar__mtr_matricula_turma') }} MTU
        ...
        INNER JOIN {{ ref('gestao_escolar__formato_avaliacao') }} FAV
            ON TUR.fav_id = FAV.id_formato_avaliacao   
        LEFT JOIN {{ ref('gestao_escolar__aluno_justificativa_falta') }} AFJ
        ...
        LEFT JOIN {{ ref('gestao_escolar__tipo_justificativa_falta') }} TJF
