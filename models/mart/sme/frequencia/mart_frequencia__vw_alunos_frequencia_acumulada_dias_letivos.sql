...
WITH frequencia_acumulada AS (
    ...
    FROM {{ ref('_raw_gestao_escolar__aluno_avaliacao_turma') }} AAT
    INNER JOIN {{ ref('_raw_gestao_escolar__avaliacao') }} AVA
        ON AAT.fav_id = AVA.fav_id
        AND AAT.ava_id = AVA.ava_id
    INNER JOIN {{ ref('_raw_gestao_escolar__tur_turma') }} TUR
        ON AAT.tur_id = TUR.tur_id
        AND TUR.tur_situacao IN (1,5)
    INNER JOIN {{ ref('_raw_gestao_escolar__calendario_anual') }} CAL
        ON TUR.cal_id = CAL.cal_id
        AND CAL.cal_ano = EXTRACT(YEAR FROM CURRENT_DATE())
    INNER JOIN {{ ref('_raw_gestao_escolar__calendario_periodo') }} CAP
    ...
    FROM {{ ref('_raw_gestao_escolar__numeroDeAulasCte') }} NUM
    LEFT JOIN (
        SELECT
            MTU.alu_id
        ...
        FROM {{ ref('_raw_gestao_escolar__turma_aula_aluno') }} TAA
        INNER JOIN {{ ref('_raw_gestao_escolar__turma_aula') }} TAU
        ...
        INNER JOIN {{ ref('_raw_gestao_escolar__mtr_matricula_turma') }} MTU
            ON TAA.id_aluno = MTU.alu_id
            AND TAA.id_matricula_turma = MTU.mtu_id
        INNER JOIN {{ ref('_raw_gestao_escolar__tur_turma') }} TUR
            ON MTU.tur_id = TUR.tur_id
        INNER JOIN {{ ref('_raw_gestao_escolar__formato_avaliacao') }} FAV
    ...
    INNER JOIN {{ ref('_raw_gestao_escolar__mtr_matricula_turma') }} MTU
            ...
            AND SQ_TA.data_aula < COALESCE(MTU.mtu_dataSaida, CURRENT_DATE())
        INNER JOIN {{ ref('_raw_gestao_escolar__tur_turma') }} TUR
            ON MTU.tur_id = TUR.tur_id
            AND TUR.tur_situacao IN (1,5)
        INNER JOIN {{ ref('_raw_gestao_escolar__calendario_anual') }} CAL
        ...
        INNER JOIN {{ ref('_raw_gestao_escolar__formato_avaliacao') }} FAV
            ON TUR.fav_id = FAV.id_formato_avaliacao   
        LEFT JOIN {{ ref('_raw_gestao_escolar__aluno_justificativa_falta') }} AFJ
        ...
        LEFT JOIN {{ ref('_raw_gestao_escolar__tipo_justificativa_falta') }} TJF
