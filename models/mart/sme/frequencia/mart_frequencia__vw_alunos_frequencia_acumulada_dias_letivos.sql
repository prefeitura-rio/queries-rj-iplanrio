...
WITH frequencia_acumulada AS (
    ...
    INNER JOIN {{ ref('raw_gestao_escolar__aca_avaliacao') }} AVA
    ...
    INNER JOIN {{ ref('raw_educacao_basica_frequencia__calendario_periodo') }} CAP
    ...
    FROM {{ ref('raw_educacao_basica_frequencia__numeroDeAulasCte') }} NUM
    LEFT JOIN (
        SELECT
            MTU.alu_id
        ...
        FROM {{ ref('raw_educacao_basica_frequencia__turma_aula_aluno') }} TAA
    ...
    INNER JOIN {{ ref('raw_gestao_escolar__mtr_matricula_turma') }} MTU
        ...
        INNER JOIN {{ ref('raw_educacao_basica_frequencia__formato_avaliacao') }} FAV
            ON TUR.fav_id = FAV.id_formato_avaliacao   
        LEFT JOIN {{ ref('raw_educacao_basica_frequencia__aluno_justificativa_falta') }} AFJ
        ...
        LEFT JOIN {{ ref('raw_educacao_basica_frequencia__tipo_justificativa_falta') }} TJF
