        ...
        select
            *,
            ...
            case
                ...
                then
                    regexp_extract(
                        resource_name, r'instances/([^/-]+(?:-[^/-]+)*-pool)'
                    )
            ...
            case
                ...
                {% endfor %}
                when project_id like '%hackathon%'
                then 'IPLANRIO'
                when project_id like '%datario%'
                then 'IPLANRIO'
                when project_id like 'rj-rec%'
                then 'RECRIO'
                when project_id like 'rj-%'
                then upper(regexp_extract(project_id, r'^rj-([^-\s]+)'))
