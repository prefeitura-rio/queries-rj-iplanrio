{% macro clean_and_cast(column_name, data_type='string', trim=false, safe=true) %}
{#
    ...
    Parâmetros:
        ...
        trim (boolean): Se deve aplicar TRIM antes do REGEXP_REPLACE. Default: false
    ...
    Exemplos:
        ...
        -- Resultado: SAFE_CAST(REGEXP_REPLACE(CAST(id_funcionario AS STRING), r'\.0$', '') AS int64)

        {{ clean_and_cast('valor', 'string', safe=false) }}
        -- Resultado: CAST(REGEXP_REPLACE(CAST(valor AS STRING), r'\.0$', '') AS string)
#}
